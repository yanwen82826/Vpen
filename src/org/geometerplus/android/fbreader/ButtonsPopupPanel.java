...
abstract class ButtonsPopupPanel extends PopupPanel implements View.OnClickListener {
	...
	public void onClick(View view) {
		...
		//UIUtil.showMessageText(myWindow.getContext(), button.ActionId);
		if( !SaveValue.IsSyncData || !"selectionBackground".equals(button.ActionId) )
			Application.doAction(button.ActionId);
		else {
			UIUtil.showMessageText(myWindow.getContext(), "目前為觀摩模式，「註記」請切換回原使用者");
			getReader().getTextView().clearSelection();
		}
