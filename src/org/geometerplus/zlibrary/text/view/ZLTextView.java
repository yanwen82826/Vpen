...
public abstract class ZLTextView extends ZLTextViewBase {
	...
	private void drawSelectionCursor(ZLPaintContext context, ZLTextSelection.Point pt) {
		...
		//final int w = ZLTextSelectionCursor.getWidth() / 2;
		final int w = ZLTextSelectionCursor.getWidth()*2/3;
		final int h = ZLTextSelectionCursor.getHeight();
		...
		context.setFillColor(new ZLColor(248,189,125), 192);
		//控制光標大小
	...
	public synchronized void paint(ZLPaintContext context, PageIndex pageIndex) {
		...
		y = getTopMargin();
		index = 0;
		//Log.i("context", "test context wid = start");
		
		for (ZLTextLineInfo info : lineInfos) {
			//
			//Log.i("context", "test context = "+info.Width);
			drawTextLine(page, info, labels[index], labels[index + 1], y);
			y += info.Height + info.Descent + info.VSpaceAfter;
			++index;
		}
		//Log.i("context", "test context wid = end");
	...
	public final synchronized int computeCurrentPage() {
		//Log.i("log", "test page = "+getCurrentCharNumber(PageIndex.current, false));
		if( SaveValue.pageIndex != getCurrentCharNumber(PageIndex.current, false) ) {
			
//			new Thread(new Runnable() {
//			    
//	            public void run() {
//	            	try {
//	            		SaveValue.draw = false;
//						Thread.sleep(500);
...
//	        }).start();
			 ...
			 //開始畫註記
			SaveValue.pageIndex = getCurrentCharNumber(PageIndex.current, false); //紀錄當前頁數
			
					
			Log.i("context", "page = "+computeTextPageNumber(getCurrentCharNumber(PageIndex.current, false)));
			final FBReaderApp fbReader = (FBReaderApp)FBReaderApp.Instance();
			fbReader.doAction(ActionCode.SELECTION_HIDE_PANEL);
			
			clearSelection();
			
			if( SaveValue.IsRepaint ) {  //目錄切換刷新
				Application.getViewWidget().reset();
				Application.getViewWidget().repaint();
				SaveValue.IsRepaint = !SaveValue.IsRepaint;
			}
			...
			if( SaveValue.IsNowSync ) {
				...
				Log.i("test", "test page = "+SaveValue.pageIndex);
			}
		}
	...
	public static int left[] = new int[100];
	public static int right[] = new int[100];
	public static int top[] = new int[100];
	public static int bottom[] = new int[100];
	public static int y;//記錄第幾行
	...
	private void drawBackgroung(
		...
		if (!highligting.isEmpty() && from != to) {
			...
			if (selectionStartArea != null
				...
				this.left[selectedy] = left;
				this.right[selectedy] = right;
				this.top[selectedy] = top;
				this.bottom[selectedy] = bottom;
				this.y = selectedy+1;
				selectedy++;
	...
	private void prepareTextLine(ZLTextPage page, ZLTextLineInfo info, int y) {
		...
		switch (getTextStyle().getAlignment()) {
			case ZLTextAlignmentType.ALIGN_RIGHT:
				...
				//int temp1 = info.Width;
				//Log.i("log", "test max info.Width = "+ temp1);
				break;
		...
		for (int wordIndex = info.RealStartElementIndex; wordIndex != endElementIndex; ++wordIndex, charIndex = 0) {
			...
			if (element == ZLTextElement.HSpace) {
				...
				if( element instanceof ZLTextWord ) {   //使文字對齊上方
					page.TextElementMap.add(new ZLTextElementArea(
							...
							x, x + width - 1, y - height + 1-5, y + descent-5
						));
					/*if( !SaveValue.w_key )
					{
						SaveValue.w_key = true;
						//Log.i("log", "text ZLTextView LineWidth = "+SaveValue.w);
						SaveValue.w = 0;
					}
					if( SaveValue.w_key )
						SaveValue.w += width-1;
